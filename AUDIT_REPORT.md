...
- **[DEV-34] Bag template delete is one-click, no confirmation — POLISH.** Clicking the X icon on a row in `/bag-templates` deletes immediately with no intermediate "Are you sure?" — and the action is irreversible from the UI. Bag templates can carry items, descriptions, sport/division mapping, and auto-assign rules; losing one accidentally is a 5–10 minute rebuild. The current `deleteTemplate` flow does call `confirm(...)` (`BagTemplatesPage.jsx:55`), but the live feel during verification was that the X icon delete fires too readily for an irreversible action on a row admins might brush past. Two reasonable patterns: (a) replace the bare browser `confirm()` with an in-app modal — same blocking shape, friendlier surface; (b) undo-toast pattern — delete fires immediately but the success toast carries an "Undo" affordance for a 5-second window. **Recommendation:** (b) — less friction for confident clicks, recoverable for the slip-finger case. Same pattern would apply to other irreversible deletes elsewhere (kit_template_items, equipment_categories, etc.) once we have the helper. **Surfaced during:** Session 7 toast verification (2026-05-04).
- **[DEV-35] Form-level inline error style hasn't followed friendlyError to the new copy pattern — POLISH.** `BagTemplatesPage` (and others) render validation errors as inline form-level text — e.g., "Add at least one item to the template." — in the older bare-line style, while toasts and the in-modal chip have moved to the WHAT-happened + WHAT-to-try pattern. The form-level path isn't broken; it's just on a different surface story. The whole "form-level error" vs. "toast" vs. "in-modal chip" surface taxonomy could use a unified treatment so future error copy doesn't fork by surface. Roll into the Session 8 tone consistency pass. Not blocking. **Surfaced during:** Session 7 toast verification (2026-05-04).
- **[DEV-36] Sister admin/EM/board-only routes still rely on in-page client-side guards — POLISH.** Routes `/treasurer`, `/safety`, `/members`, `/settings`, `/bag-templates`, `/team-bags`, `/locations`, `/board` have client-side role guards inside the page component (e.g., `BagTemplatesPage:26-31`'s `useEffect` that toasts + navigates) rather than route-level role gates. Two failure modes vs. the route-gate pattern: (a) brief flash of unauthorized content while the `useEffect` runs (~100-300ms) — not a security leak (RLS handles real data access), but a "this product knows what it's doing" delta; (b) inconsistent error UX across routes (some toast, some redirect, some show inline "no permission" UI). Extend the DEV-31 `ProtectedRoute roles=` pattern to cover these routes uniformly. Sized for Session 9 — straightforward but touches enough files to deserve its own commit. **Surfaced during:** Session 8 DEV-31 verification (2026-05-04).
- **[DEV-37] TicketDetailPage activity feed comment-event branch renders the actor's full name where the status-event branch correctly renders "You" — POLISH.** After DEV-33 wired `resolveActorLabel` into the status-event branch, the comment-event branch still shows the comment-author's full name regardless of whether the viewing user is the author. Could be intentional (matches comment-author conventions on Slack, GitHub, Linear — third-person comment headers regardless of viewer identity) or unintentional (helper just isn't wired into the comment-card render path). Confirm intent and align if needed; if intentional, document the convention so the next person reading the code doesn't "fix" it. **Surfaced during:** Session 8 DEV-33 verification (2026-05-04).
- **[DEV-38] Trigger-button vs modal-title register divergence — CONVENTION (do NOT "fix"). Severity: none.** ⚠️ This entry is recorded *protectively* — it documents an intentional pattern so future readers don't mistake it for an inconsistency to close. Across create-flow surfaces, trigger buttons and modal titles use deliberately different registers: trigger buttons stay terse imperative ("+ Add field", "+ Add member", "+ Add check") to fit compact UI alongside other controls; modal titles take the article-on-create form ("Add a field", "Add a member", "Add a background check") to read as warmth in the modal's wider visual register. This matches established SaaS conventions (Stripe, GitHub, Linear). The divergence is intentional and should NOT be "fixed" by aligning the strings — the registers serve different functional roles. Specific cases also include noun specificity expansion (e.g., "+ Add check" trigger → "Add a background check" modal title) where compact-context terseness on the trigger expands to disambiguating precision in the modal heading. **Documented:** Session 9 tone audit, post-Cluster 1 commit 2 (2026-05-04). **Action:** none — recording convention, not a finding.
- **[DEV-39] friendlyError pattern is opt-in; structural enforcement recommended — POLISH.** Three separate Session 7/9 sweeps have surfaced the same `error.message` raw-leak anti-pattern across ProfilePage, ticket-surface call sites, bag-surface call sites, and EquipmentPage modal subtree (Receive/Transfer/Remove/Audit). Pattern is opt-in: callers must remember to wrap with `friendlyError(error)`, and the build/tests don't catch when they don't. The third sweep finding new instances on a previously-untouched page (EquipmentPage modal subtree, found in Session 9 tone re-scan) confirms this is structural, not bad luck. **Recommendation:** add an ESLint rule that flags `addToast(error.message ...)` and `setError(error.message ...)` patterns, OR enforce the wrap at the `lib/errors.js` boundary by exporting a `safeToast(addToast, error)` (or similar) that internally calls `friendlyError`. Either approach makes the wrap mandatory at the call site instead of polite. **Surfaced during:** Session 7 ticket sweep, Session 9 ProfilePage + EquipmentPage re-scan (2026-05-04). **Sized for:** Session 10 or a dedicated tooling commit.
- **[DEV-40] /board position cards render the assigned member's role as raw enum — POLISH.** Position cards on `/board` render the assigned member's role as the raw user_roles enum string ("admin", "treasurer") in the card pill row. Separate render path from the InviteModal's `allRoleOptions` (which correctly uses `formatRoleLabel` per the Session 9 T-03 boundary fix). Wrap the position-card pill render through `formatRoleLabel` for consistency — same anti-pattern as MembersPage inline display, which T-03 already closed. **Surfaced during:** Session 9 T-03 verification (2026-05-04). **Effort:** one-line fix; the helper is already imported in BoardPage.
- **[DEV-41] (POLISH) — CLOSED:** SafetyPage tab labels "Background Checks" / "First Aid Kits" → sentence case ("Background checks" / "First aid kits") per T-07. Surfaced during Session 9 cluster 2 verification walk; closed in this same commit alongside the UX-09 expansion. **Status:** closed 2026-05-04.
- **[DEV-42] SafetyPage First aid kits tab empty state lacks the bordered-card wrapper — POLISH.** Incidents and Background checks tabs render their empty state inside `<div style={{background:'white', border:'1px solid var(--gray-200)', borderRadius:'var(--radius-lg)', overflow:'hidden'}}>`; the Kits tab uses a `grid` parent instead (correct for populated kit cards but missing the wrapped frame when empty). Result: the empty-state content sits left-aligned to the page padding edge instead of centered inside a card, and the three sibling tabs don't share identical visual chrome. Likely a missing wrapper element or different className in the Kits branch of the SafetyPage tab render. **Surfaced during:** Session 9 Cluster 3 verification walk (2026-05-04). **Effort:** two-line fix when convenient.
- **[DEV-43] `.mp-remove-mobile` tap target at 36px (under HIG) — POLISH.** The mobile Members "Remove" button (M-04 fix) is `min-height: 36px`, below the 44pt HIG floor. Bumping in isolation creates a height mismatch with the adjacent `.role-select` (which is sized to fit its content). A future tap-target session should address `.mp-remove-mobile` + `.role-select` together as a paired bump — both grow to 44pt with consistent vertical alignment. Cluster 3's `.btn-icon` global fix won't catch this class because it's a different selector. **Surfaced during:** Session 10 Cluster 2 verification (2026-05-04). **Effort:** small — paired CSS bump in MembersPage's inline `<style>` block + verify the role select grows cleanly alongside.
- **[DEV-44] `/locations` page h1 diverges from sidebar nav + document.title — POLISH.** Page h1 reads "Storage locations" while the sidebar nav label and the browser tab title both read "Locations". Three surfaces should agree — either all "Storage locations" or all "Locations". **Recommendation:** align to the shorter "Locations" since (a) sidebar/tab title are the more user-facing canonical names, (b) "Storage" is implied by the inventory-management context. T-07's sentence-case cascade landed correctly but didn't catch this surface-naming mismatch (it was a casing pass, not a content pass). **Surfaced during:** Session 10 Cluster 3 M-03 verification (2026-05-04). **Effort:** single-line fix.
...
- **[OPS-02] Working rule — keyboard-nav verification on layout work.** Session 10 Cluster 1's table-as-cards on Bag templates (commit `ddb89ff`) shipped with a clickable `<div className="bt-card">` that's keyboard-unreachable. The Cluster 1 verification budget at the time covered visual rendering and 44pt tap-target measurement, but not keyboard nav. Two human checks and one AI check missed it. **Going forward:** any commit that adds new clickable surfaces (cards, custom buttons, `role="button"` divs, etc.) should include a keyboard-nav verification step — Tab through the new surface, confirm each interactive element receives focus, confirm Enter/Space activates it. This is in addition to existing visual + tap-target checks. Adding this to the working rules is the structural fix; the per-surface code fix lives in Session 11 cluster A-03. **Lesson recorded:** Session 11 a11y static analysis (2026-05-04). **Status:** process change, not a code finding.
