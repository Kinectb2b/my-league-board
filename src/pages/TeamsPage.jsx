              ...
              return (
                <div key={sport.id} className={`sport-section ${dragOverSport === sport.id ? 'sport-drag-over' : ''} ${dragSport?.id === sport.id ? 'sport-dragging' : ''}`}
                  ...
                  <div className="sport-header" draggable onDragStart={e => { setDragSport(sport); e.dataTransfer.effectAllowed = 'move' }} onDragEnd={() => { setDragSport(null); setDragOverSport(null) }}>
                    ...
                    {canEdit && <div className="sport-header-actions"><button className="btn-icon-sm btn-icon-danger" onClick={e => { e.stopPropagation(); deleteSport(sport.id, sport.name) }} aria-label={`Delete ${sport.name}`}>✕</button></div>}
                  </div>
                          ...
                          <div className="team-list">
                              ...
                              return (
                                <div key={team.id} className="team-row">
                                  ...
                                  {canEdit && <div className="team-row-actions">
                                    <button className="btn-icon-sm" onClick={() => duplicateTeam(team)} aria-label={`Duplicate ${team.name}`} title="Duplicate">⧉</button>
                                    <button className="btn-icon-sm" onClick={() => setEditingTeam(team)} aria-label={`Edit ${team.name}`}>✎</button>
                                    <button className="btn-icon-sm btn-icon-danger" onClick={() => deleteTeam(team.id, team.name)} aria-label={`Delete ${team.name}`}>✕</button>
                                  </div>}
                                </div>
                              )
                            })}
                            {quickAddDivisionId === div.id && (
                              <div className="quick-add-row">
                                ...
                                <button className="btn-icon-sm" onClick={() => { setQuickAddDivisionId(null); setQuickAddName('') }} aria-label="Cancel adding team">✕</button>
                              </div>
                            )}
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                {tItems.map((item, idx) => (
                  <div key={idx} style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
                    ...
                    <button type="button" className="btn-icon-sm btn-icon-danger" onClick={() => removeTItem(idx)} aria-label="Remove this item">✕</button>
                  </div>
                ))}
              </div>
            )}
