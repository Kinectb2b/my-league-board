...
export default function TeamsPage() {
  ...
  async function fetchAll() {
    ...
    const [t, d, s, sp, b, tmpl] = await Promise.all([
    ...
    setTeams(t.data||[]); setDivisions(d.data||[]); setSeasons(s.data||[]); setSportTypes(sp.data||[]); setBags(b.data||[]); setTemplates(tmpl.data||[]); setLoading(false)
  }
...
function AddTeamModal({ orgId, divisions, sportTypes, onDone, onClose }) {
  const [name, setName] = useState(''); const [did, setDid] = useState(divisions[0]?.id||''); const [color, setColor] = useState('#1a472a'); const [sub, setSub] = useState(false); const [err, setErr] = useState('')
  const grouped = sportTypes.map(s => ({ sport: s, divs: divisions.filter(d => d.sport_type_id === s.id) })).filter(g => g.divs.length > 0)
  async function go(e) { e.preventDefault(); setSub(true); const { error } = await supabase.from('teams').insert({ organization_id: orgId, name, division_id: did, color }); if (error) setErr(error.message); else onDone(); setSub(false) }
  return (<div className="modal-overlay" onClick={onClose}><div className="modal" onClick={e => e.stopPropagation()}><div className="modal-header"><h2>Add team</h2><button className="btn-icon" onClick={onClose}>✕</button></div><form onSubmit={go} className="modal-form"><div className="form-group"><label>Name *</label><input type="text" value={name} onChange={e => setName(e.target.value)} required /></div><div className="form-row"><div className="form-group"><label>Division *</label><select value={did} onChange={e => setDid(e.target.value)} required>{grouped.map(g => <optgroup key={g.sport.id} label={g.sport.name}>{g.divs.map(d => <option key={d.id} value={d.id}>{d.name}</option>)}</optgroup>)}</select></div><div className="form-group"><label>Color</label><input type="color" value={color} onChange={e => setColor(e.target.value)} /></div></div>{err && <div className="form-error">{err}</div>}<div className="modal-actions"><button type="button" className="btn-secondary" onClick={onClose}>Cancel</button><button type="submit" className="btn-primary" disabled={sub}>{sub ? 'Adding...' : 'Add'}</button></div></form></div></div>)
}

function EditTeamModal({ team, divisions, onDone, onClose }) {
  const [name, setName] = useState(team.name); const [did, setDid] = useState(team.division_id); const [color, setColor] = useState(team.color||'#1a472a'); const [sub, setSub] = useState(false); const [err, setErr] = useState('')
  async function go(e) { e.preventDefault(); setSub(true); const { error } = await supabase.from('teams').update({ name, division_id: did, color }).eq('id', team.id); if (error) setErr(error.message); else onDone(); setSub(false) }
